...
int main(int argc, char **argv) {
  ...
  float *a, *b, *c, *c_gpu, *c_tiled_gpu;
  float *d_a, *d_b, *d_c;
  a = (float *)malloc(size);
  b = (float *)malloc(size);
  c = (float *)malloc(size);
  c_gpu = (float *)malloc(size);
  c_tiled_gpu = (float *)malloc(size);
  cudaMalloc(&d_a, size);
  cudaMalloc(&d_b, size);
  cudaMalloc(&d_c, size);
  for (int i = 0; i < n; i++) {
  ...
  printf("Time for simple matrix multiplication: %f ms\n", time_spent);
  cudaMemcpy(c_gpu, d_c, size, cudaMemcpyDeviceToHost);

  // Tiled GPU matmul
  start = clock();
  tiled_matmul<<<dimGrid, dimBlock>>>(d_a, d_b, d_c, n);
  cudaDeviceSynchronize();
  end = clock();
  time_spent = (1000 * (double)(end - start) / CLOCKS_PER_SEC);
  printf("Time for tiled matrix multiplication: %f ms\n", time_spent);
  cudaMemcpy(c_tiled_gpu, d_c, size, cudaMemcpyDeviceToHost);

  // Check results
  for (int i = 0; i < n * n; i++) {
    assert(c[i] == c_gpu[i]);
    assert(c[i] == c_tiled_gpu[i]);
  }
